...
SWEP.AmmoEnt = "item_ammo_357_ttt"									-- SWEP.AmmoEnt determines the ammo pickup that will feed this gun.
...
function SWEP:UseBolt()
	...
	if self.Owner:GetAmmoCount( self.Weapon:GetPrimaryAmmoType() ) > 0 then
		...
		if SERVER and self.Weapon != nil then 
			...
			if getWeaponClassName(self.Owner:GetActiveWeapon()) == self.Gun and self.BoltAction then
				...
				timer.Simple(boltactiontime, 
					...
					if SERVER and self.Weapon != nil then
						...
						elseif !self.Owner:KeyDown(IN_ATTACK2) and getWeaponClassName(self.Owner:GetActiveWeapon()) == self.Gun then
							self.Owner:RemoveAmmo(1, self.Primary.Ammo, false) -- out of the frying pan
							self.Weapon:SetClip1(self.Weapon:Clip1() + 1) --  into the fire
							self.Weapon:SetNextPrimaryFire(CurTime() + .1)
						end
			...
			-- else if getWeaponClassName(self.Owner:GetActiveWeapon()) == self.Gun and 
