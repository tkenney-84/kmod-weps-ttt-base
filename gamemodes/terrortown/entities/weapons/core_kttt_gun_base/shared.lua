...
SWEP.AmmoEnt = "item_box_buckshot_ttt"								-- SWEP.AmmoEnt determines the ammo pickup that will feed this gun.
...
function SWEP:Reload()
		...
		if self.Weapon:Clip1() <= 0 and self.Owner:GetAmmoCount(self.Primary.Ammo) <= 0 then return end
		...
		timer.Simple(waitdammit + .1,
				...
				self.Weapon:SetNWBool("Reloading", false)
				if self.Owner:KeyDown(IN_ATTACK2) and getWeaponClassName(self.Owner:GetActiveWeapon()) == self.Gun then
				...
				elseif self.Owner:KeyDown(IN_SPEED) and getWeaponClassName(self.Owner:GetActiveWeapon()) == self.Gun then
...
-- //copy this...
		...
		self.IronSightsPos = self.RunSightsPos                                  -- Hold it down
		self.IronSightsAng = self.RunSightsAng                                  -- Hold it down
		self:SetIronsights(false, self.Owner)                                    -- Set the ironsight true - KENNEY NOTE: This made running not work. Come on man. Tying anims to ironsights for sprinting? Really?
		self.Owner:SetFOV( 0, 0.3 )
		self.DrawCrosshair = false
		end                                                            
...
function getWeaponClassName(activeWeapon)
	local currWeapPreStr = tostring(activeWeapon)
	local startIndex = string.find(currWeapPreStr,"_") + 1
	local prefix = "kttt"
	local currWeapStr = string.sub(currWeapPreStr,startIndex,string.len(currWeapPreStr) - 1)
	local checkStr = prefix.."_"..currWeapStr
	return checkStr
end